...
// Current log level
#define USBLOG_LEVEL    USBLOG_NONE
...
int usbhw_transfer_out(uint8_t pid, int device_addr, int endpoint, int handshake, uint8_t request, uint8_t *tx, int tx_length)
{
    ...
    LOG(USBLOG_ERR, ("USB: Unknown OUT response (%x)\n", resp));
...
//-----------------------------------------------------------------
void usbhw_hub_enable(int speed, int enable_sof)
{
    uint32_t val;

    LOG(USBLOG_ERR, ("HW: Enable root hub (speed=%d)\n", speed));

    // Host control settings
    val = 0;
    val |= ((speed & 0x3) << USB_CTRL_PHY_XCVRSELECT_SHIFT);
...
{    
    ...
    // Reset duration is 10 ms
    if (!is_sim()) {
      usbhw_timer_sleep(12); // was 11
      usbhw_timer_sleep(100);
    }
    ...
    // Enable SOF
    usbhw_hub_enable(usbhw_hub_full_speed_device() ? 1 : 2, 1);
    if (!is_sim())
      usbhw_timer_sleep(100);
...
static unsigned int *uart = (unsigned int *)0x20000000;
