...
// Current log level
#define USBLOG_LEVEL    USBLOG_ENUM

#define USBLOG(l,a)     do { if (l <= USBLOG_LEVEL) printf a; } while (0)
//#define USBLOG(l,a)     do { printf a; } while (0)
...
static int usb_setup_packet(int device_address, uint8_t request_type, uint8_t request, uint16_t value, uint16_t index, uint16_t length)
{
    ...
    res = usbhw_transfer_out(PID_SETUP, device_address, 0, 1, PID_DATA0, usb_request, idx);
    printf("res = %x\n", res);
...
int usb_configure_device(struct usb_device *dev, int device_address)
{
    ...
    res = usb_get_descriptor(dev, DESC_DEVICE, 0, _desc_buffer, 8); //sizeof(struct UsbDeviceDescriptor));
    if (res < 0)
    {
